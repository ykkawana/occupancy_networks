...
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn10_target_n_4096_no_overlap_reg_20200502_001739/part_assignment_20200504_000717.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn10_target_n_4096_no_overlap_reg_20200502_001739/debug_part_assignment_20200504_150747.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_target_n_4096_no_overlap_reg_20200502_041512/20200504_152221part_assignment_debug_.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn10_target_n_4096_no_overlap_reg_20200502_001739/debug_part_assignment_20200504_153812.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/bspnet_pn30_20200501_191145/debug_part_assignment_20200504_214302.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_no_regs_no_normal_20200502_202018/debug_part_assignment_20200504_224022.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_no_regs_20200502_104902/debug_part_assignment_20200504_224809.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/bspnet_pn30_20200501_191145/debug_part_assignment_20200504_231307.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/bspnet_pn30_20200501_191145/debug_part_assignment_20200504_232535.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_no_regs_no_normal_20200502_202018/debug_part_assignment_20200504_233200.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_no_regs_20200502_104902/debug_part_assignment_20200504_233700.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_target_n_4096_no_overlap_reg_20200502_041512/debug_part_assignment_20200504_153005.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_target_n_4096_no_overlap_reg_20200502_041512/part_assignment_20200504_235613.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/bspnet_pn30_20200501_191145/part_assignment_20200504_234734.yaml'
config_path = '/home/mil/kawana/workspace/occupancy_networks/out/submission/eval/img/pnet_finetue_only_transition_cceff10_pn30_target_n_4096_no_overlap_reg_20200502_041512/part_assignment_20200505_014711.yaml'
...
counts_np = pickle.load(open(cfg['test']['semseg_part_assigned_pkl'], 'rb'))
...
count_dict = {key: count.argmax(axis=1) for key, count in counts_np.items()}
ious = []
# %%
for it, data in enumerate(tqdm(val_loader)):
    #it = 0
    #data = next(iter(tqdm(val_loader)))
    if data is None:
        print('Invalid data.')
        continue
    # Get index etc.
    ...
    with torch.no_grad():
        if is_pnet:
            ...
            if bsp_use_cache:
                vertex_attributes = np.load(
                    os.path.join(
                        ...
                        '{}_primitive_vertex_visibility.npz'.format(
                            modelname)))
                vertices = torch.from_numpy(
                    vertex_attributes['points']).float().to(device).unsqueeze(
                        0)
                vertices_mask = torch.from_numpy(
                    vertex_attributes['vertex_visibility']).to(
                        device).unsqueeze(0)
            else:
                with torch.no_grad():
            ...
            if not bsp_use_cache:
                surface_vertices = gen_helper.roty90(surface_vertices,
                                                     inv=True)

                imnet_pointcloud = data.get('pointcloud.imnet_points').to(
                    device).float()
                imnet_pointcloud = gen_helper.roty90(imnet_pointcloud,
                                                     inv=True)
    ...
    pred_labels = np.take(count_dict[class_id], sidx.cpu().numpy(), 0)
    gt_labels = labels_long.cpu().numpy()

    if metric_mode == 'iou':

        part_num = len(set(list(class_names_to_part_ids[class_name].values())))
        ...
        for original_label, idx_idx in class_names_to_part_ids[
                class_name].items():
            tp = np.logical_and(pred_labels == idx_idx,
                                gt_labels == original_label).sum()
            tps[idx_idx] += tp

            fp = np.logical_and(pred_labels == idx_idx,
                                gt_labels != original_label).sum()
            fps[idx_idx] += fp

            fn = np.logical_and(pred_labels != idx_idx,
                                gt_labels == original_label).sum()
            fns[idx_idx] += fn
        ...
        tmp = {}
        for part_idx in tps:
            iou = tps[part_idx] / (tps[part_idx] + fps[part_idx] +
                                   fns[part_idx])
            tmp['part_{}'.format(part_idx)] = iou
        ious[-1].update(tmp)
    else:
...
out_path = os.path.join(
    base_eval_dir, '{}eval_part_assignment_{}.pkl'.format(
        ('debug_' if debug else ''), date_str))
for label, hist in label_counts.items():
    hist1 = (counts_np[label] / counts_np[label].sum(0, keepdims=True)) + 1e-7
    hist2 = (label_counts[label] /
             label_counts[label].sum(0, keepdims=True)) + 1e-7
    for n in range(hist1.shape[1]):
        print('n', n, scipy.stats.wasserstein_distance(hist1[:, n], hist2[:,
                                                                          n]))
df.to_pickle(out_path)
cfg['test']['semseg_part_assigned_pkl'] = out_path
yaml.dump(cfg, open(out_path.replace('.pkl', '.yaml'), 'w'))
print(df.groupby('class_name').mean().T.mean())
print('mean', df.groupby('class_name').mean().T.mean().mean())
# %%
