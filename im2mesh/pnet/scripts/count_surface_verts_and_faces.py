...
out_path = os.path.join(mesh_dir_path, 'surface_verts_faces_count.pkl')
...
def get_verts_faces_stats(inputs):
    ...
    for it in tqdm(indices):
        ...
        if is_explicit_mesh:
            ...
            v1_out = np.take(vis, v1)
            v2_out = np.take(vis, v2)
            v3_out = np.take(vis, v3)
            face_out = v1_out | v2_out | v3_out
            return_dicts.append({
                'class_id': class_id,
                'class_name': class_name,
                'modelname': modelname,
                'verts_num': len(vis),
                'faces_num': len(face_out),
                'verts_in': len(vis) - vis.sum(),
                'faces_in': len(face_out) - face_out.sum(),
                'verts_out': vis.sum(),
                'faces_out': face_out.sum()
            })
        else:
            return_dicts.append({
                'class_id': class_id,
                'class_name': class_name,
                'modelname': modelname,
                'verts_num': len(all_mesh.vertices),
                'faces_num': len(all_mesh.faces),
                'verts_in': 0.,
                'faces_in': 0.,
                'verts_out': len(all_mesh.vertices),
                'faces_out': len(all_mesh.faces)
            })
