...
parser.add_argument(
    ...
    'with exit code 2.')
parser.add_argument('--target_id', default=None, help='class_id/model_id')
parser.add_argument('--data_from',
                    choices=['bsp', 'kaolin'],
                    help='data source for overfit')
...
input_idx = random.sample(range(len(pointcloud)),
                          k=min(cfg['data']['pointcloud_n'], len(pointcloud)))
...
points_idx = random.sample(range(len(points)),
                           k=min(cfg['data']['points_subsample'],
                                 len(pointcloud)))
batch_update = {
    'points': torch.from_numpy(points[points_idx, :]).unsqueeze(0),
    'points.occ':
    torch.from_numpy(values[points_idx, :]).unsqueeze(0).squeeze(-1),
    'pointcloud': torch.from_numpy(pointcloud[pointcloud_idx, :]).unsqueeze(0),
    'inputs': torch.from_numpy(pointcloud[input_idx, :]).unsqueeze(0)
}
val_batch_update = {
    'points_iou':
...
val_batch.update(val_batch_update)
data_vis = batch
while True:
    epoch_it += 1
    ...
    if True:
        ...
        if validate_every > 0 and (it % validate_every) == 0 and it > 0:
            eval_dict = trainer.evaluate(None, batch=val_batch)
